...
public class CameraMode_Free : ScriptableObject, ICameraMode
{
    ...
    void MoveCameraLocal(_Camera camera, Vector3 target)
    {
        if (_Input.fastCamera)
            camera.transform.Translate(_Input.inputDirection * freeFastSpeed * Time.deltaTime / Time.timeScale);
        else
            camera.transform.Translate(_Input.inputDirection * freeNormalSpeed * Time.deltaTime / Time.timeScale);
    }
    void RotateCamera(_Camera camera, Vector3 target)
    {
        Vector3 newPosition = camera.transform.position;
        rotationX += _Input.mouseAxis.x * camera.mouseSensitivity * Time.deltaTime;
        rotationY += _Input.mouseAxis.y * camera.mouseSensitivity * Time.deltaTime;
